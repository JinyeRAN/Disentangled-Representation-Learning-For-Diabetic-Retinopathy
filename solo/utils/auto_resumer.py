...
class AutoResumer:
    ...
    def add_and_assert_specific_cfg(cfg: DictConfig) -> DictConfig:
        ...
        cfg.auto_resume.max_hours = omegaconf_select(cfg, "auto_resume.max_hours", default=36000)
    ...
    def find_checkpoint(self, cfg: DictConfig):
        ...
        if os.path.exists(self.checkpoint_dir):
            for folder in os.listdir(self.checkpoint_dir):
                folder_dir = os.path.join(self.checkpoint_dir, folder)
                checkpoint_file = os.path.join(folder_dir, self.LAST_FILENAME + self.FILE_EXTENSION)
                if os.path.exists(checkpoint_file):
                    ...
                    if current_time - creation_time < self.max_hours:
                        ck = Checkpoint(
                            ...
                            args=os.path.join(folder_dir, "args.json"),
